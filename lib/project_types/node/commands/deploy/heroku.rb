    ...
    class Deploy
      class Heroku < ShopifyCli::SubCommand
        ...
        def call(*)
          ...
          install_message =
            @ctx.message(@ctx.windows? ? 'node.deploy.heroku.installing_windows' : 'node.deploy.heroku.installing')
          ...
          if (app_name = heroku_service.app)
            ...
            app_type =
              CLI::UI::Prompt.ask(@ctx.message('node.deploy.heroku.app.no_apps_found')) do |handler|
          ...
          if branches.length == 1
            ...
            branch_to_deploy =
              CLI::UI::Prompt.ask(@ctx.message('node.deploy.heroku.git.what_branch')) do |handler|
