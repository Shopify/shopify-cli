  ...
  module Tasks
    class GetApps < ShopifyCli::Task
      ...
      def apps_from_organizations(organizations)
        organizations.flat_map { |organization| apps_owned_by_organization(organization) }
      end

      def apps_owned_by_organization(organization)
        return [] unless organization.key?('apps') && organization['apps'].any?

        organization['apps'].map { |app| Converters::AppConverter.from_hash(app, organization) }
      end
    end
  end
end
