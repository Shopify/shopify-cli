    ...
    MESSAGES = {
      script: {
        error: {
          ...
          invalid_config:
            "Can't change the configuration values because %1$s is missing or " \
          ...
          shop_script_conflict_cause:
            'Another app in this store has already enabled a script ' \
          ...
          build_script_suggestion:
            'Root package.json needs a script named build, which' \
        ...
        enable: {
          ...
          HELP
          info:
            '{{*}} A script always remains enabled until you disable it - even after pushing ' \
