  ...
  module Generator
    class Markdown
      ...
      def render(data)
        ...
        data.each { |cls| File.write("#{cls.filename}.md", class_renderer.result(cls.instance_eval { binding })) }
      ...
      def build_classes
        classes =
          @store.all_classes_and_modules.map do |klass|
        ...
        classes.reject do |klass|
          klass.comment.empty? && klass.constants.empty? && klass.class_methods.empty? &&
            klass.instance_methods.empty? && klass.attributes.empty? && klass.extended.empty? && klass.included.empty?
        end
      end
