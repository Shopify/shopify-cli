  ...
  module Tasks
    class SelectOrgAndShop < ShopifyCli::Task
      ...
      def response(organization_id, shop_domain)
        { organization_id: organization_id, shop_domain: shop_domain }
      end
      ...
      def get_organization(organization_id)
        @organization ||=
          if !organization_id.nil?
            ...
            org_id =
              CLI::UI::Prompt.ask(ctx.message('core.tasks.select_org_and_shop.organization_select')) do |handler|
                organizations.each do |o|
                  handler.option(ctx.message('core.partners_api.org_name_and_id', o['businessName'], o['id'])) do
                    o['id']
                  end
                end
              end
      ...
      def get_shop_domain(organization)
        valid_stores =
          organization['stores'].select do |store|
