...
module ShopifyCli
  module Tasks
    class EnsureEnv < ShopifyCli::Task
      def call(ctx, regenerate: false, required: %i[api_key secret])
        @ctx = ctx
        env_data =
          begin
      ...
      def fetch_org
        Shopifolk.act_as_shopify_organization if Shopifolk.check && wants_to_run_against_shopify_org?
        ...
        org_id =
          if orgs.count == 1
            ...
            CLI::UI::Prompt.ask(@ctx.message('core.tasks.ensure_env.organization_select')) do |handler|
              orgs.each do |org|
                handler.option(@ctx.message('core.partners_api.org_name_and_id', org['businessName'], org['id'])) do
                  org['id']
                end
              end
            end
      ...
      def get_app(org_id, apps)
        if apps.count == 1
          ...
          type =
            CLI::UI::Prompt.ask(@ctx.message('core.tasks.ensure_env.app_type.select')) do |handler|
      ...
      def get_shop(shops, id)
        if shops.count == 1
          ...
          shop =
            CLI::UI::Prompt.ask(@ctx.message('core.tasks.ensure_env.development_store_select')) do |handler|
      ...
      def write_env(env_data, org)
        ...
        Resources::EnvFile.new(env_data).write(@ctx)
      end
    end
  end
end
