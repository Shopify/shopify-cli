    ...
    MESSAGES = {
      ...
      core: {
        ...
        monorail: { consent_prompt: <<~MSG },
          ...
          MSG
        oauth: {
          error: { timeout: 'Timed out while waiting for response from Shopify' },
          location: { admin: 'development store', partner: 'Shopify Partners account' },
        ...
        project: { error: { not_in_project: <<~MESSAGE } },
        ...
        system: {
          ...
          HELP
          error: { unknown_option: "{{x}} {{red:unknown option '%s'}}" },
