    ...
    class DevServer
      ...
      class << self
        def start(
          ...
          instance.setup(ctx, root, host, theme, port, poll, editor_sync, stable, mode)
          instance.start
        end
      ...
      def setup(ctx, root, host, theme_identifier, port, poll, editor_sync, stable, mode)
        @ctx = ctx
        @root = root
        @host = host
      ...
      def not_found_error_message
        ctx.message("theme.serve.theme_not_found", identifier)
      end
