...
class JavyTest < Minitest::Test
  ...
  ASSET_FILENAME = "javy-x86_64-%s-v0.2.1.gz"

  def setup
    super
    FileUtils.mkdir_p(::Javy::BIN_FOLDER)
    FileUtils.mkdir_p(::Javy::HASH_FOLDER)
    File.write(File.join(::Javy::ROOT, "version"), "v0.2.1")
  end
  ...
  def stub_executable_download_request(os, executable_sha)
    ...
    stub_request(:get, "https://github.com/Shopify/javy/releases/download/v0.2.1/#{asset_filename}")
      .to_return(
        status: 200,
        headers: {
  ...
  def simulate_broken_asset_link
    stub_request(:get, "https://github.com/Shopify/javy/releases/download/v0.2.1/#{ASSET_FILENAME % "macos"}")
