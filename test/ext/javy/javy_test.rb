...
class JavyTest < Minitest::Test
  ...
  ASSET_FILENAME = "javy-x86_64-%s-#{Javy::VERSION}.gz"

  def setup
    super
    FileUtils.mkdir_p(::Javy::BIN_FOLDER)
    FileUtils.mkdir_p(::Javy::HASH_FOLDER)
    File.write(File.join(::Javy::ROOT, "version"), Javy::VERSION)
  end
  ...
  def stub_executable_download_request(os, executable_sha)
    ...
    stub_request(:get, "https://github.com/Shopify/javy/releases/download/#{Javy::VERSION}/#{asset_filename}")
      .to_return(
        status: 200,
        headers: {
  ...
  def simulate_broken_asset_link
    stub_request(:get, "https://github.com/Shopify/javy/releases/download/#{Javy::VERSION}/#{ASSET_FILENAME % "macos"}")
