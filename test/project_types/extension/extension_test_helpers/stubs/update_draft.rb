    ...
    module Stubs
      module UpdateDraft
        ...
        def stub_update_draft(registration_id:, config:, extension_context: nil, api_key: 'FAKE_API_KEY')
          stub_partner_req(
            Tasks::UpdateDraft::GRAPHQL_FILE,
            variables: {
              api_key: api_key,
            ...
            resp: { data: yield(registration_id, config, extension_context) },
          )
        end
        ...
        def stub_update_draft_failure(errors:, **args)
          stub_update_draft(**args) { { extensionUpdateDraft: { Tasks::UserErrors::USER_ERRORS_FIELD => errors } } }
        end
      end
    end
  end
