...
module Script
  module Commands
    class EnableTest < MiniTest::Test
      ...
      def test_help
        ShopifyCli::Context.expects(:message).with('script.enable.help', ShopifyCli::TOOL_NAME)
      ...
      def test_calls_application_enable_with_configuration_properties
        expected_configuration = { entries: [{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }] }

        expect_successful_enable(expected_configuration)

        capture_io { perform_command(config_props: 'key1:value1,key2:value2') }
      end

      def test_with_config_props_with_spaces
        expected_configuration = { entries: [{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'with spaces' }] }

        expect_successful_enable(expected_configuration)

        capture_io { perform_command(config_props: 'key1:value1 , key2:with spaces') }
      end
      ...
      def test_calls_application_enable_with_configuration_file
        ...
        capture_io { perform_command(config_file_path: 'enable_config_file.yml') }
      end
      ...
      def test_calls_application_enable_with_configuration_file_and_properties_override
        ...
        capture_io { perform_command(config_props: 'key1:overriddenValue', config_file_path: 'enable_config_file.yml') }
      end

      def test_calls_application_enable_with_invalid_configuration_file
        File.open('enable_config_file.yml', 'w+') { |file| file.write("key1 \"value1\"\nkey2: \"value2\"") }

        Script::UI::ErrorHandler.expects(:pretty_print_and_raise).with(instance_of(Errors::InvalidConfigYAMLError))
