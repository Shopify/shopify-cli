...
describe Script::Layers::Infrastructure::AssemblyScriptTaskRunner do
  ...
  let(:extension_point_config) do
    { 'assemblyscript' => { "package": '@shopify/extension-point-as-fake', "version": '*', "sdk-version": '*' } }
  end
  ...
  before { Script::ScriptProject.stubs(:current).returns(script_project) }
  ...
  describe '.build' do
    ...
    it 'should raise an error if no build script is defined' do
      ...
      assert_raises(Script::Layers::Infrastructure::Errors::BuildScriptNotFoundError) { subject }
    end
    ...
    it 'should raise an error if the generated web assembly is not found' do
      File.expects(:read).with('package.json').once.returns(JSON.generate(package_json))
      ctx.expects(:file_exist?).with('build/foo.wasm').once.returns(false)

      ctx.expects(:capture2e).with('npm run build').once.returns(['output', mock(success?: true)])
    ...
    it 'should trigger the compilation process' do
      ...
      ctx.expects(:file_exist?).with('build/foo.wasm').once.returns(true)
    ...
    it 'should raise error without command output on failure' do
      ...
      ctx.stubs(:capture2e).returns([output, mock(success?: false)])

      assert_raises(Script::Layers::Domain::Errors::ServiceFailureError, output) { subject }
    end
  end
  ...
  describe '.dependencies_installed?' do
    ...
    before { FileUtils.mkdir_p('node_modules') }
    ...
    it 'should error if major version is different' do
      ...
      msg = "NPM packages out of date: #{package_name}"
      error =
        assert_raises Script::Layers::Infrastructure::Errors::PackagesOutdatedError do
          subject
        end
      assert_equal msg, error.message
    end
  ...
  describe '.install_dependencies' do
    ...
    describe 'when node version is above minimum' do
      it 'should install using npm' do
        ctx.expects(:capture2e).with('node', '--version').returns(['v14.5.1', mock(success?: true)])
        ctx
          .expects(:capture2e)
    ...
    describe 'when node version is below minimum' do
      it 'should raise error' do
        ctx.expects(:capture2e).with('node', '--version').returns(['v14.4.0', mock(success?: true)])
  ...
  def stub_npm_outdated(output)
    ctx.stubs(:capture2e).with('npm', 'outdated', '--json', '--depth', '0').returns([output.to_json, mock])
  end
