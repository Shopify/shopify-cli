...
describe Script::Layers::Infrastructure::AssemblyScriptProjectCreator do
  ...
  describe '.setup_dependencies' do
    ...
    it 'should write to npmrc' do
      ...
      context.expects(:system).with('npm', '--userconfig', './.npmrc', 'config', 'set', 'engine-strict', 'true')
      subject
    end
    ...
    it 'should fetch the latest extension point version' do
      ...
      extension_point.expects(:sdks).times(4).returns({ ts: sdk })
  ...
  describe '.bootstrap' do
    ...
    it 'should delegate the bootstrapping process to the language toolchain' do
      context
        .expects(:capture2e)
        .with(
          "npx --no-install shopify-scripts-toolchain-as bootstrap --from #{extension_point.type} --dest #{
            script_name
          }",
        )
