    ...
    module GenerateTests
      class WebhookTest < MiniTest::Test
        ...
        def test_with_existing_param
          @context
            .expects(:system)
            .with(regexp_matches(Regexp.new("^.*#{BIN_REGEX}\\\" webhook APP_UNINSTALLED$")))
            .returns(mock(success?: true))
          Node::Commands::Generate::Webhook.new(@context).call(%w[APP_UNINSTALLED], '')
        end
        ...
        def test_with_incorrect_param_expects_ask
          ...
          @context
            ...
            .with(regexp_matches(Regexp.new("^.*#{BIN_REGEX}\\\" webhook APP_UNINSTALLED$")))
            .returns(mock(success?: true))
          Node::Commands::Generate::Webhook.new(@context).call(%w[create_webhook_fake], '')
        end
        ...
        def test_with_selection
          ...
          @context
            .expects(:system)
            .with(regexp_matches(Regexp.new("^.*#{BIN_REGEX}\\\" webhook PRODUCT_CREATE$")))
            .returns(mock(success?: true))
