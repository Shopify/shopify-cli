...
describe ShopifyCli::ScriptModule::Infrastructure::DeployPackageRepository do
  ...
  let(:configuration) do
    ShopifyCli::ScriptModule::Domain::Configuration.new("config_id", "configuration.schema", configuration_schema, "config code")
  end
  ...
  describe ".create_deploy_package" do
    subject { deploy_package_repository.create_deploy_package(script, bytecode) }

    it "should create a deploy package" do
      FakeFS.with_fresh do
        deploy_package = subject
        assert_equal bytecode, File.read(build_file)
        assert_equal bytecode, deploy_package.bytecode
      end
    end
  end
    ...
    describe "when script exists" do
      ...
      it "should return the deploy package when valid script and wasm exist" do
        FakeFS.with_fresh do
          ...
          File.write(build_file, bytecode)

          assert_equal build_file, subject.id
        end
      end

      it "should raise WasmNotFoundError when wasm does not exist" do
        FakeFS.with_fresh do
          FileUtils.mkdir_p(build_base)
          assert_raises(ShopifyCli::ScriptModule::Domain::WasmNotFoundError) { subject }
        end
      end
    end
