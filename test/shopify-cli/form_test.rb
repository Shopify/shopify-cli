...
module ShopifyCli
  class FormTest < MiniTest::Test
    ...
    def test_that_all_attributes_are_defined
      ...
      form = TestForm.ask(@context, %w[a b c d], three: 'one', four: 'two')
      assert_equal('a', form.one)
      assert_equal('b', form.two)
      assert_equal(%w[c d], form.xargs)
    ...
    def test_that_the_form_returns_nil_if_shopify_abort_is_raised
      io =
        capture_io do
          form = TestForm.ask(@context, %w[a b c d], three: 'raise', four: 'two')
          assert_nil(form)
        end
