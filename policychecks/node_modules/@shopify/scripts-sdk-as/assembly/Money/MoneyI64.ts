...
/**
 * A `Moneyi64` represents an amount of money in a given currency.
 *
 * This is exactly the same as the `Money` class but doesn't use OverflowableInteger.
 * It will replace it soon enough.
 *
 ...
 *   `<=`
 *  */
export class MoneyI64 {
  private readonly subunits: i64;
  ...
  private constructor(amount: f64, currency: Currency) {
    ...
    this.subunits = currency.unitsToSubunits(amount);
  }
   ...
   */
  static fromAmount(amount: f64, currency: Currency): MoneyI64 {
    return new MoneyI64(amount, currency);
  }
   ...
   */
  round(step: f64, mode: RoundingMode): MoneyI64 {
    if (step <= 0) {
    ...
    switch (mode) {
      case RoundingMode.UP:
        rounded = roundTowardsInfinity(this.subunits, stepSubunits);
        break;
      case RoundingMode.DOWN:
        rounded = roundTowardsNegativeInfinity(this.subunits, stepSubunits);
        break;
      case RoundingMode.NEAREST:
        rounded = roundNearestAwayFromZero(this.subunits, stepSubunits);
        break;
    }

    return MoneyI64.fromAmount(this.currency.subunitsToUnits(rounded), this.currency);
  }
  ...
  @operator('*')
  private __multiply(factor: f64): MoneyI64 {
    return MoneyI64.fromAmount(this.units * factor, this.currency);
  }

  // Relative operations can only be performed on scalar values
  @operator('/')
  private __divide(divisor: f64): MoneyI64 {
    const units = this.units;
    const result = units / divisor;

    assertValidFloatValue(result, units.toString() + ' / ' + divisor.toString());

    return MoneyI64.fromAmount(result, this.currency);
  }

  @operator('==')
  private __equals(other: MoneyI64): bool {
    if (this.units == 0 && other.units == 0) return true;
    return this.units == MoneyI64.convertUnits(other, this.currency);
  }

  @operator('!=')
  private __nequals(other: MoneyI64): bool {
    return !this.__equals(other);
  }

  // Normalizes to and takes the currency of the left operand
  @operator('+')
  private __add(other: MoneyI64): MoneyI64 {
    return MoneyI64.fromAmount(this.units + MoneyI64.convertUnits(other, this.currency), this.currency);
  }

  // Normalizes to and takes the currency of the left operand
  @operator('-')
  private __subtract(other: MoneyI64): MoneyI64 {
    return MoneyI64.fromAmount(this.units - MoneyI64.convertUnits(other, this.currency), this.currency);
  }

  @operator('>')
  private __gt(other: MoneyI64): bool {
    const normalizedUnits = MoneyI64.convertUnits(other, this.currency);
    return this.units > normalizedUnits;
  }

  @operator('>=')
  private __gte(other: MoneyI64): bool {
    const normalizedUnits = MoneyI64.convertUnits(other, this.currency);
    return this.units >= normalizedUnits;
  }

  @operator('<')
  private __lt(other: MoneyI64): bool {
    const normalizedUnits = MoneyI64.convertUnits(other, this.currency);
    return this.units < normalizedUnits;
  }

  @operator('<=')
  private __lte(other: MoneyI64): bool {
    const normalizedUnits = MoneyI64.convertUnits(other, this.currency);
    return this.units <= normalizedUnits;
  }

  private static convertUnits(from: MoneyI64, toCurrency: Currency): f64 {
  ...
  private initializeUnitsIfUnset(): void {
    if (!this.__units) {
      this.__units = new Box(this.currency.subunitsToUnits(this.subunits));
    }
  }
