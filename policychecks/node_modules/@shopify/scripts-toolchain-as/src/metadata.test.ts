...
describe('getMetadata', () => {
  describe('setting schemaVersions for legacy eps', () => {
    it('should error if there are no modules', () => {
      stubFetchingModules([]);
      expect(() => getMetadata()).toThrow('Expected to have at least one EP NPM package installed');
    });
    ...
    it('should error if missing metadata files', () => {
      ...
      expect(() => getMetadata()).toThrow(
        'NPM package extension-point-as-foo does not have metadata associated with it',
      );
    });
    ...
    it('should error if metadata file is missing schemaVersion property', () => {
      ...
      expect(() => getMetadata()).toThrow('NPM package extension-point-as-foo does not have a schemaVersion defined');
    });
  });
  ...
  describe('setting schemaVersions for domain eps', () => {
    it('should throw an error if there are no modules', () => {
      stubFetchingModules([]);
      expect(() => getMetadata('payment-methods', 'checkout')).toThrow(
        'Expected to have @shopify/scripts-checkout-apis installed',
      );
    });

    it('should throw an error if only the ep option is set', () => {
      stubFetchingModules([]);
      expect(() => getMetadata('payment-methods')).toThrow(
        'When working with domain packages, both --ep and --domain options are required',
      );
    });

    it('should throw an error if only the domain option is set', () => {
      stubFetchingModules([]);
      expect(() => getMetadata(undefined, 'checkout')).toThrow(
        'When working with domain packages, both --ep and --domain options are required',
      );
    });
    ...
    it('should set the version for the specified ep', () => {
      ...
      const metadata = getMetadata('foo', 'checkout');
    ...
    it('should convert dashes to underscores in package names', () => {
      ...
      const metadata = getMetadata('foo-bar-baz', 'checkout');
...
function stubFetchingModules(entries: fs.Dirent[]) {
  mockedFs.readdirSync.mockReturnValue(entries);
}

function stubMetadataJson(values: {[ep: string]: string}, ep?: string, domain?: string) {
...
type CreateDirentParams = {
  name: string;
  isDirectory: boolean;
  isSymlink: boolean;
};

function createDirectory(name: string): fs.Dirent {
  return createDirent({name, isDirectory: true, isSymlink: false});
}

function createSymlink(name: string): fs.Dirent {
  return createDirent({name, isDirectory: false, isSymlink: true});
}

function createFile(name: string): fs.Dirent {
  return createDirent({name, isDirectory: false, isSymlink: false});
}

function createDirent({name, isDirectory, isSymlink}: CreateDirentParams): fs.Dirent {
  return {
    name,
    isDirectory() {
      return isDirectory;
